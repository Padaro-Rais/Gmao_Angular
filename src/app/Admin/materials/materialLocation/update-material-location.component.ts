...
@Component({
  selector: 'app-update-material-location',
  template: `
    <h6 id="titre1">Configuration</h6>
    <h2 id="titre"><b>EMPLACEMENT MATERIELS</b></h2>
...
export class UpdateMaterialLocationComponent implements OnInit {
  id: any
  data: any
  ...
  getUserById() {
    ...
    this.service.getUserById(this.id).subscribe((res) => {
      this.data = res
      this.location = this.data.item
      console.log(this.location)
    })
  }

  updateData() {
    this.service.updateData(this.id, this.location).subscribe(
      (res) => {
